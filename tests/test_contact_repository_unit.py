...
@pytest.fixture
def mock_session():
    """Fixture for creating a mock database session object."""
    return AsyncMock(spec=AsyncSession)


@pytest.fixture
def contact_repository(mock_session):
    """Fixture for creating a ContactRepository instance with a mock session."""
...
async def test_get_contacts(contact_repository, mock_session, user):
    ...
    mock_result.scalars.return_value.all.return_value = [
        Contact(
            id=1, name="John", surname="Doe", email="john.doe@example.com", user=user
        )
    ]
    mock_session.execute = AsyncMock(return_value=mock_result)

    contacts = await contact_repository.get_contacts(
        name="", surname="", email="", skip=0, limit=10, user=user
    )
...
async def test_update_contact(contact_repository, mock_session, user):
    ...
    existing_contact = Contact(
        id=1, name="Old Name", surname="Old Surname", email="old@example.com", user=user
    )
...
async def test_remove_contact(contact_repository, mock_session, user):
    ...
    existing_contact = Contact(
        id=1, name="To Delete", surname="Person", email="delete@example.com", user=user
    )
